...
const FriendCard: React.FC<{ friend: Friend }> = ({ friend }) => {
  return (
    <div className="glass border border-white/10 rounded-2xl p-4 relative group overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-violet-500/0 via-violet-500/0 to-cyan-400/0 group-hover:from-violet-500/10 group-hover:to-cyan-400/10 transition"></div>

      <div className="relative flex items-start gap-3">
        {/* Avatar */}
        <div className="relative">
          <span className="relative flex shrink-0 overflow-hidden rounded-full h-14 w-14 ring-2 ring-white/10">
            <img className="aspect-square h-full w-full object-cover" src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${friend.seed}&backgroundColor=b6e3f4,c0aede,d1d4f9,ffd5dc,ffdfbf&backgroundType=gradientLinear`} alt={`${friend.name}'s avatar`} />
          </span>
      ...
      <div className="relative mt-4 grid grid-cols-3 gap-2">
        <button className="inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 shadow hover:bg-primary/90 px-3 h-9 rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white text-xs hover:opacity-90">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {stats.map((stat, index) => (
          <div key={index} className="glass border border-white/10 rounded-2xl p-4 relative overflow-hidden">
            <div className={`absolute -top-6 -right-6 h-20 w-20 rounded-full bg-gradient-to-br ${stat.gradient} opacity-30 blur-2xl`}></div>
            <div className="relative">
