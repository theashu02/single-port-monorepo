...
import { chatRequested, selectChatPeer, selectChatPhase, type ChatPhase } from "@/lib/redux/slices/chatSlice";
...
const UserCard: React.FC<{
...
}> = memo(({ user, isCurrentUser, canChat, isActivePeer, isLocalChatLocked, chatPhase, onChat }) => {
  ...
  const isWaitingForThisUser = isActivePeer && chatPhase === "awaiting-accept";
      ...
      : isChattingWithThisUser
        ...
        : isTargetBusy
          ...
          : isExpiredForThisUser
            ? "Timed out"
            : "Chat";
  const buttonDisabled = !canChat || isCurrentUser || isTargetBusy || isWaitingForThisUser || isLocalChatLocked;
    ...
    <div className="glass border border-white/10 rounded-2xl p-4 relative group overflow-hidden">
      ...
      <div className="relative flex flex-col items-center text-center">
        <div className="relative">
          ...
          <span className={`absolute -bottom-0.5 right-1 h-4 w-4 rounded-full ring-2 ring-[#0c0a18] ${user.isBusy ? "bg-amber-300" : "bg-emerald-400 pulse-ring"}`} />
        </div>

        <p className="mt-3 text-sm font-bold truncate w-full">{user.name}</p>
        <p className="text-[11px] text-white/50 truncate w-full">{isCurrentUser ? (user.isBusy ? "You are in a chat" : "You are online") : user.isBusy ? "In a chat" : "Using the app now"}</p>
        ...
        <div className="flex gap-1 mt-2 flex-wrap justify-center">
          ...
          {!isCurrentUser && user.isBusy && <span className="text-[10px] px-2 py-0.5 rounded-md bg-amber-400/10 text-amber-200 border border-amber-400/15">Busy</span>}
          {isCurrentUser && <span className="text-[10px] px-2 py-0.5 rounded-md bg-white/5 text-white/60">You</span>}
        </div>

        <Button type="button" onClick={handleChat} disabled={buttonDisabled} className={buttonClassName}>
...
export const OnlinePeoplePage: React.FC = () => {
  ...
  const busyUsersCount = users.reduce((count, user) => count + (user.isBusy && user.id !== currentUserId ? 1 : 0), 0);
  const isConnected = status === "connected";
  const isLocalChatLocked = chatPhase === "awaiting-accept" || chatPhase === "incoming-invite" || chatPhase === "open";
    ...
    <>
      <div className="flex flex-col w-full h-full p-4 lg:p-6 gap-4 min-w-0 overflow-hidden">
          ...
          <div className="relative flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="grid grid-cols-2 gap-2 min-w-[220px]">
              ...
              <div className="rounded-2xl glass border border-white/10 p-3">
                ...
                <p className="mt-1 text-sm font-bold text-white/80">{busyUsersCount}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {filters.map((filter) => (
            <button key={filter.id} type="button" onClick={() => setActiveFilter(filter.id)} className={`shrink-0 px-4 h-8 rounded-full text-xs font-semibold capitalize transition ${activeFilter === filter.id ? "bg-linear-to-r from-violet-500 to-cyan-400 text-white" : "bg-white/5 text-white/60 hover:bg-white/10 border border-white/10"}`}>
              {filter.label}
            </button>
          ))}
        </div>
