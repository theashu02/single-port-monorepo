...
const ChatWindow: React.FC = memo(() => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-80 sm:w-96 flex flex-col rounded-3xl overflow-hidden border border-white/10 glass shadow-2xl animate-in slide-in-from-bottom-4 fade-in duration-300" role="region" aria-label={`Chat with ${peer.name}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-white/5">
        <div className="flex items-center gap-3">
          <div className="relative">
            <span className="flex h-8 w-8 rounded-full bg-linear-to-br from-violet-500 to-cyan-400 items-center justify-center text-xs font-bold text-white select-none">{peer.name.charAt(0).toUpperCase()}</span>
        ...
        <button type="button" onClick={handleClose} className="h-7 w-7 rounded-full flex items-center justify-center bg-white/5 hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/30" aria-label="Close chat">
      ...
      <div className="flex-1 overflow-y-auto max-h-72 px-4 py-3 scrollbar-hide">
        {messages.length === 0 && <p className="text-center text-xs text-white/30 mt-8">Say hello to {peer.name} 👋</p>}
        {messages.map((msg) => (
          <MessageRow key={msg.id} msg={msg} isMine={msg.fromId !== "__system__" && msg.fromId === currentUserId} />
        ))}
        <div ref={bottomRef} />
      </div>
